        ...
        <div class="row">
            <div class="col-xl-12 order-xl-1">
                <div class="card bg-secondary shadow">
                    ...
                    <div class="card-body">
                        <form method="post" action="/category/{{ $data->id }}" autocomplete="off">
                            @csrf
                            {{method_field('PUT')}}
                                ...
                                <div class="form-group{{ $errors->has('name') ? ' has-danger' : '' }}">
                                    ...
                                    <select name="parent_id" class="form-control form-control-alternative{{ $errors->has('name') ? ' is-invalid' : '' }}"  id="">
                                        ...
                                        @foreach($parents as $parent)
                                            <option value="{{ $parent->id }}" @if($data->parent() ? $data->parent()->id == $parent->id: '')  selected  @endif><b>{{ $parent->name }}</b></option>
                                        @endforeach
                                    </select>
                                    @if ($errors->has('name'))
                                        <span class="invalid-feedback" role="alert">
